...
function formatAntigravityResumeCommand() {
  return "/antigravity:rescue --resume";
}

function appendActiveJobsTable(lines, jobs) {
  lines.push("Active jobs:");
  lines.push("| Job | Kind | Status | Phase | Elapsed | Conversation ID | Summary | Actions |");
  lines.push("| --- | --- | --- | --- | --- | --- | --- | --- |");
...
function pushJobDetails(lines, job, options = {}) {
  ...
  if (job.threadId) {
    lines.push(`  Antigravity conversation ID: ${job.threadId}`);
    lines.push(`  Resume: ${formatAntigravityResumeCommand()}`);
  }
...
export function renderFreeTextReviewResult(result, meta) {
  const output = (result.finalMessage ?? "").trim();
  const lines = [
    `# Antigravity ${meta.reviewLabel}`,
    "",
    `Target: ${meta.targetLabel}`,
  ...
  if (output) {
    ...
    lines.push("Antigravity review completed without output.");
  } else {
...
export function renderStoredJobResult(job, storedJob) {
  ...
  const resumeCommand = threadId ? formatAntigravityResumeCommand() : null;
  ...
  if (isStructuredReviewStoredResult(storedJob) && storedJob?.rendered) {
    ...
    return `${output}\nAntigravity conversation ID: ${threadId}\nResume: ${resumeCommand}\n`;
  }
  ...
  if (threadId) {
    lines.push(`Antigravity conversation ID: ${threadId}`);
